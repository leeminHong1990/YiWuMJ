			...
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				...
				if (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative) {
					if (switches.appstore_check == true) {
						...
						cutil.lock_ui();
						jsb.reflection.callStaticMethod("IAPOcBridge", "startPurchWithID:completeHandle:", "12_cards", funcId);
					} else {
						cutil.get_pay_url(1);
					}
			...
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				...
				if (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative) {
					if (switches.appstore_check == true) {
						...
						cutil.lock_ui();
						jsb.reflection.callStaticMethod("IAPOcBridge", "startPurchWithID:completeHandle:", "52_cards", funcId);
					} else {
						cutil.get_pay_url(2);
					}
				} else if (cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) {
			...
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				...
				if (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative) {
					if (switches.appstore_check == true) {
						...
						cutil.lock_ui();
						jsb.reflection.callStaticMethod("IAPOcBridge", "startPurchWithID:completeHandle:", "115_cards", funcId);
					} else {
						cutil.get_pay_url(3);
					}
				} else if (cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) {
