...
var CreateAgentRoomUI = UIBase.extend({
    ...
    initRoomScroll: function () {
		...
		function playing_btn_event(sender, eventType) {
            if (eventType === ccui.Widget.TOUCH_ENDED) {
               ...
               h1global.entityManager.player().getPlayingRoomInfo();
            }
		}
		...
		function complete_btn_event(sender, eventType) {
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                h1global.entityManager.player().getCompleteRoomInfo();
            }
		}
    ...
    initCreateRoom:function(){
        ...
        function create_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				var king_mode = parameters["king_mode"];
				var round_num = parameters["round_num"];
				var round_mode = parameters["round_mode"];
                var base_score_mode = parameters["base_score_mode"];
                var score = parameters["score"];
                var job = parameters["job"];//job即承包，默认为1，即默认承包以及反承包
                ...
                h1global.entityManager.player().createRoom(game_mode, pay_mode, king_mode, round_num, round_mode, base_score_mode, max_lose, score, 1, discard_seconds, hand_prepare, 1);
            }
        }
    ...
    updatePlayingItem: function (item, data) {
        ...
        var round_mode = data['round_mode'];
        ...
        if (round_mode == const_val.NORMAL_ROUND_MODE) {
            round_label.setString(score + '分');
        } else {
        ...
        if (cur_round >= 1) {
            ...
            var label_str = null;
        	if(round_mode == const_val.NORMAL_ROUND_MODE){
                label_str = "游戏中(" + cur_round.toString() + ')';
			}else{
        		label_str = "游戏中(" + cur_round.toString() + '/' + max_round.toString() + ')';
			}
            ...
            if (player_list.length < 4) {
                lackStr = cc.formatStr("%d等%d,", player_list.length, 4 - player_list.length);
            }
            var share_title = switches.gameName + ' 房间号【' + room_id.toString() + '】' + "【代开房】";
            var share_desc = lackStr + cutil.get_agent_room_desc(data);
			...
			dismiss_btn.addTouchEventListener(function (sender, eventType) {
				if (eventType === ccui.Widget.TOUCH_ENDED) {
					h1global.entityManager.player().agentDismissRoom(room_id);
				}
			});
		}
	},
