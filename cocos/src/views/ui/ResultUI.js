...
var ResultUI = UIBase.extend({
	...
	show_by_info:function(finalPlayerInfoList, curGameRoom, serverSitNum){
		...
		this.show(function(){
            ...
            if(!((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative) || switches.TEST_OPTION)) {
                ...
                var clubId = curGameRoom.club_id;
                var clubStr = "";
                var title = "";
                if(clubId > 0){
                    clubStr = " 亲友圈号【"+clubId+"】";
                    ...
                    title = clubStr +'房间号【' + curGameRoom.roomID.toString() + '】';
                }
            ...
            if (serverSitNum > 0) {
                let p0 = self.player_panels[0].getPositionX();
                let p1 = self.player_panels[serverSitNum].getPositionX();
                self.player_panels[0].setPositionX(p1);
                self.player_panels[serverSitNum].setPositionX(p0);
            }
            let roomInfoLabel = self.result_panel.getChildByName("roominfo_label");
            roomInfoLabel.setString(cutil.convert_timestamp_to_datetime_exsec(new Date() / 1000) + "   房间号：" + curGameRoom.roomID.toString());
            var clubStr2 = "";
			if (curGameRoom.club_id > 0) {
				clubStr2 = "    亲友圈号：" + curGameRoom.club_id;
                roomInfoLabel.setString(cutil.convert_timestamp_to_datetime_exsec(new Date() / 1000) + clubStr2);
			}
    ...
    update_player_info: function (serverSitNum, finalPlayerInfo, win_idx, curGameRoom) {
        ...
        // var pong_num = finalPlayerInfo["pong_times"];    //碰牌次数
        var job_score = finalPlayerInfo["job_score"];    //场外承包
        var give_win_times = finalPlayerInfo["give_win_times"];    //放炮次数
        ...
        var job_score_label = ccui.helper.seekWidgetByName(cur_player_info_panel, "job_score_label");
        job_score_label.setString(job_score.toString());
        var give_win_times_label = ccui.helper.seekWidgetByName(cur_player_info_panel, "give_win_times_label");
        give_win_times_label.setString(give_win_times.toString());
