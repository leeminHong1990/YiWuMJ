...
var GameRoomUI = UIBase.extend({
	...
	init_curplayer_panel:function(){
		...
		if(player.serverSitNum == curPlayerSitNum && (player.curGameRoom.handTilesList[player.serverSitNum].length)%3==2 && !this.rootUINode.getChildByName("operation_panel").isVisible()){
    ...
    update_extra_xia_panel: function () {
        if (!this.is_show) {
            return;
        }
        ...
        let extra_score_panel = this.rootUINode.getChildByName("extra_score_panel");
        for (var i = 0; i < player.curGameRoom.player_num; i++) {
            var index = player.server2CurSitNum(i);
            var score_panel = extra_score_panel.getChildByName("score_panel" + index);
            score_panel.setVisible(player.curGameRoom.extra_xia_list[i] > 0);
			let label = score_panel.getChildByName("num_label");
            label.setString('X' + player.curGameRoom.extra_xia_list[i])
        }
    },
	...
	update_player_info_panel:function(serverSitNum, playerInfo){
        ...
        var lose_panel = cur_player_info_panel.getChildByName('lose_panel')
        if (lose_panel) {
        ...
        if (!owner_img.editorOrigin) {
            owner_img.editorOrigin = owner_img.getPosition();
        }
		if(player.curGameRoom.dealerIdx == serverSitNum){
            owner_img.setPositionY(owner_img.editorOrigin.y - 16);
		}else{
            owner_img.setPositionY(owner_img.editorOrigin.y);
		}

        var state_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "state_img");
        cur_player_info_panel.reorderChild(state_img, 4);

        var mai_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "mai_img");
        cur_player_info_panel.reorderChild(mai_img, 3);
		let curGameRoom = player.curGameRoom;
		if(curGameRoom){
            mai_img.setVisible(curGameRoom.extra_mai_list[serverSitNum] > 0);
		}
		this.update_player_discard_king(serverSitNum)
    },
    ...
    update_player_discard_king: function (serverSitNum) {
        if (!this.is_show) {return;}
        if(serverSitNum < 0 || serverSitNum > 3){
            return;
        }
        ...
        var idx = player.server2CurSitNum(serverSitNum);
        var cur_player_info_panel = this.rootUINode.getChildByName("player_info_panel" + idx.toString());
		if(!cur_player_info_panel) {return;}

        var king_panel = ccui.helper.seekWidgetByName(cur_player_info_panel, "king_panel");

        let curGameRoom = player.curGameRoom;
        let num = curGameRoom.discard_king_total[serverSitNum];
        king_panel.setVisible(num > 0);
        var num_label = king_panel.getChildByName('king_label');
        num_label.setString('X' + num);
        king_panel.setVisible(false);
    },
	...
	init_player_tile_panel:function(){
		...
		function addMoving_tile(tile){
			...
			if(player.isKingTile(tile.tileNum)){
        ...
        function discard_tile(tile, discard_pos) {
            let player = h1global.entityManager.player();
            if (player){
                if (player.canDiscard(tile.tileNum)) {
        ...
        function hand_tile_event(tile, touch_mode){
            ...
            if (player2 && !player2.canDiscard(tile.tileNum)){
            	return;
			}
    ...
    mark_hand_king_tiles: function (serverSitNum, hand_tiles) {
        var player = h1global.entityManager.player();
        ...
        for(var i = 0; i < hand_tiles.length; i++){
            ...
            if (player.curGameRoom.kingTiles.indexOf(hand_tiles[i]) >= 0) {
                ...
                if (player.isOpLimit(serverSitNum, const_val.OP_DISCARD, tile_img.tileNum)) {
                    this.mark_disable_tile(tile_img);
                } else {
    ...
    mark_disable_tile: function (tile_img) {
        tile_img.color = const_val.mark_disable_color;
    },
	...
	mark_same_tiles:function(tileNum){
        ...
        var player = h1global.entityManager.player();
        if (player && player.curGameRoom && player.curGameRoom.kingTiles.indexOf(tileNum) >= 0) {
			cc.log("mark tile is king");
			return;
        }
        // 财神在其他地方标记
        if(player.isKingTile(tileNum)){
            return;
        }
	...
	cancel_mark_same_tiles:function(){
		...
		for(var i = 0; i < 4; i++){
			...
			for(var j = 0; j< 20; j++){
                ...
                if (player && player.isKingTile(tile_img.tile_num)) {
                    continue;
                }
				this.cancel_tile_color(tile_img);
			}
	...
	startBeginAnim:function(startTilesList, diceList, dealerIndex){
		...
		this.throwTheDice(diceList, dealerIndex, playAnimation);
	},
    ...
    throwTheDice:function (diceList, dealerIndex, cbkFunc) {
        // is_left = 1 or -1
        ...
        function playCbk() {
            dice_anim_node.runAction(cc.sequence(
                cc.callFunc(function () {
                    showDice(diceDouble[0], diceDouble[1], dealerIndex === 3 ? -1 : 1)
                }),
                cc.delayTime(0.5),
				cc.callFunc(function () {
    ...
    update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        let opLimit = player.curGameRoom.op_limit;
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                ...
                if(opLimit[serverSitNum]){
                    if (player.isOpLimit(serverSitNum, const_val.OP_DISCARD, handTilesList[i])) {
                        if(!player.isKingTile(handTilesList[i])){
                            this.cancel_tile_color(tile_img);
                            this.mark_disable_tile(tile_img);
                        }
                    }else{
            ...
            if (i === 0 && player.curGameRoom.discard_king_total[serverSitNum] > 0) {
                this.update_player_hand_king(idx, tile_img)
            }
        }
        ...
        if(serverSitNum == player.serverSitNum){
            this.mark_hand_king_tiles(serverSitNum, handTilesList)
        }
    },

    update_player_hand_king:function (index, tile_img) {
        cc.log("not impl update_player_hand_king");
    },
	...
	update_player_up_tiles: function (serverSitNum) {
        ...
        for (var i = 0; i < curUpNum; i++) {
            ...
            if (ops) {
                ...
                var before_king = ops[0]["king"] == 0;
                if (before_king) {
                    from_img.loadTexture("res/ui/GameRoomUI/arrow.png");
                } else {
                    from_img.loadTexture("res/ui/GameRoomUI/arrow_red.png");
                }
	...
	update_player_discard_tiles:function(serverSitNum){
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
                ...
                if(player.isKingTile(tile_img.tile_num)){
                	this.mark_king_tile(tile_img);
				}else{
                	this.cancel_tile_color(tile_img);
				}
    ...
    playOperationEffect:function(opId, serverSitNum, tile){
        ...
        } else if(opId == const_val.OP_PIAO){
            this.playOperationFunc(curSitNum, "piao");
        ...
        }else if(opId == const_val.OP_NORMAL_WIN){
            this.playOperationFunc(curSitNum, "win");
        }
    },
    ...
    play_result_anim: function (player_info_list, callback) {
        var player = h1global.entityManager.player();
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
            ...
            cutil.tileSort(handTiles, curGameRoom.kingTiles);
        ...
        let anim = cc.Node.create();
        anim.setName('result_anim_node');
        this.addChild(anim);
        anim.runAction(cc.sequence(cc.delayTime(1), cc.callFunc(callback), cc.removeSelf()))
    },
    ...
    update_kingtile_panel: function () {
        var player = h1global.entityManager.player();
        if (!this.is_show || !player) {
            return
        }
        var kings = player.curGameRoom.kingTiles;
        ...
        if (kings && kings.length > 0) {
            ...
            var mahjong_img = tile_img.getChildByName("mahjong_img");
            mahjong_img.ignoreContentAdaptWithSize(true);
            mahjong_img.loadTexture("Mahjong/mahjong_big_" + kings[0].toString() + ".png", ccui.Widget.PLIST_TEXTURE);
            mahjong_img.setVisible(true);
            this.mark_king_tile(tile_img)
            kingtile_panel.setVisible(true);
        } else {
            kingtile_panel.setVisible(false);
        }
    },
    ...
    show_operation_panel:function(){
        ...
        this.adapter_operation_panel(operation_panel, operation_panel.editorOrigin, h1global.entityManager.player().getCanWinTiles().length > 0);
