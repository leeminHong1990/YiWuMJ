var GameRoomInfoUI = MultipleLayoutUI.extend({
	ctor: function() {
		this._super();
		this.resourceFilename = cc.sys.localStorage.getItem("GAME_ROOM_UI") == const_val.GAME_ROOM_2D_UI ? GameRoomInfoUI.ResourceFile2D : GameRoomInfoUI.ResourceFile3D;
		this.setLocalZOrder(const_val.GameRoomInfoZOrder);
	},
    ...
    update_round: function () {
        ...
        if (player && player.curGameRoom) {
            if (player.curGameRoom.round_mode == const_val.NORMAL_ROUND_MODE) {
                this.round_label.setString(player.curGameRoom.curRound.toString());
            } else {
                this.round_label.setString(player.curGameRoom.curRound.toString() + "/" + player.curGameRoom.maxRound.toString());
            }
        }
    },
