...
var PlaybackGameRoom = cc.Node.extend({
    ...
    doPost: function (command) {
        let serverSitNum = command.serverSitNum;
        let player = h1global.entityManager.player();
        player.postOperation(serverSitNum, command.aid, command.tileList);
        if (command.aid == const_val.OP_DRAW_WIN) {
            let handTiles = player.curGameRoom.handTilesList[serverSitNum];
            let kingTiles = player.curGameRoom.kingTiles;
            let classifyList = cutil.classifyWindTiles(handTiles, kingTiles);
            let handTilesButKing = classifyList[1];
            let handKingNum = classifyList[0].length;

            let isBaotou = false;
            //白板顶财神
            for (var i = 0; i < handTilesButKing.length; i++) {
                if (handTilesButKing[i] === const_val.DRAGON_WHITE) {
                    handTilesButKing[i] = player.curGameRoom.kingTiles[0]
                }
            }
            let finalTile = command.tileList[0];
            if (handKingNum > 0) {
                let insteadFinalTile = finalTile === const_val.DRAGON_WHITE ? kingTiles[0] : finalTile;

                if (kingTiles.indexOf(finalTile) >= 0) {
                    if (cutil.get_count(handTiles, finalTile) >= 2) {
                        isBaotou = cutil.canNormalWinWithKing3N(handTilesButKing, handKingNum - 2);
                    }
                } else {
                    let newHandTilesButKing = handTilesButKing.concat();
                    newHandTilesButKing.splice(newHandTilesButKing.indexOf(insteadFinalTile), 1);
                    isBaotou = cutil.canNormalWinWithKing3N(newHandTilesButKing, handKingNum - 1)
                }
            }
            if (isBaotou) {
                player.postExtOperation(serverSitNum, command.aid, command.tileList, [0]);
            }else{
                player.postExtOperation(serverSitNum, command.aid, command.tileList, [1]);
            }
        }
    },
