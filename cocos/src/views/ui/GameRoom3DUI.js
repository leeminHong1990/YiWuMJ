...
var GameRoom3DUI = GameRoomUI.extend({
    ...
    update_player_hand_king: function (index, tile_img) {
        ...
        if(index ===0){
            tile_img.loadTexture("res/ui/GameRoomUI/hand_king.png");
            mahjong_img.setVisible(false);
        }else if(index ===1){
            tile_img.loadTexture("res/ui/GameRoomUI/left_king.png");
        }else if(index ===2){
            tile_img.loadTexture("Mahjong/mahjong_tile_player_hand.png", ccui.Widget.PLIST_TEXTURE);
            tile_img.ignoreContentAdaptWithSize(true);
            tile_img.setPositionY(152);
            tile_img.color = const_val.mark_king_color;
            mahjong_img.setPosition(cc.p(tile_img.getContentSize().width * 0.5, tile_img.getContentSize().height * 0.45));
            mahjong_img.loadTexture("Mahjong/mahjong_big_" + h1global.entityManager.player().curGameRoom.kingTiles[0] + ".png", ccui.Widget.PLIST_TEXTURE);
            mahjong_img.setVisible(true);
            mahjong_img.setScaleY(-1);
            mahjong_img.setScaleX(-1);
    ...
    get_play_discard_anim_config:function(serverSitNum, index, onlyTilePointer){
        ...
        if(index == 0){
            ...
            if(!onlyTilePointer){
                cur_delay_tile_position = cc.p(640,260);
            ...
            if(!onlyTilePointer){
                cur_delay_tile_position = cc.p(840,420);
            ...
            if(!onlyTilePointer){
                cur_delay_tile_position = cc.p(640,520);
            ...
            options.pointerPosition = cc.p(41, 78);
            if(!onlyTilePointer){
                cur_delay_tile_position = cc.p(440,420);
