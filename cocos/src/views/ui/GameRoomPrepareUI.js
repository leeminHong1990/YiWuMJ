...
var GameRoomPrepareUI = UIBase.extend({
    ctor:function() {
        ...
        this.talk_img_num = 0;
        this.setLocalZOrder(const_val.GameRoomPrepareZOrder);
    },

    initUI:function(){
        var player = h1global.entityManager.player();
        this.gameprepare_panel = this.rootUINode.getChildByName("gameprepare_panel");
        if (player && player.curGameRoom.curRound > 0) {
            //因为2D的GAME_ROOM_UI值为0,3D的值为1,所以这里加1
            this.gameprepare_panel.setVisible(false);
            this.gameprepare_panel = this.rootUINode.getChildByName("gameprepare" + (parseInt(cc.sys.localStorage.getItem("GAME_ROOM_UI")) + 2).toString() + "d_panel");
        }
        ...
        wxinvite_btn.addTouchEventListener(function(sender, eventType){
            if (eventType == ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                if (player) {
                    var clubId = player.curGameRoom.club_id;
                    var clubStr = "";
                    var share_title="";
                    if(clubId > 0){
                        clubStr = " 亲友圈【"+clubId+"】";
                        ...
                        share_title = switches.gameName + clubStr + ' 房间号【' + player.curGameRoom.roomID.toString() + '】';
                    }
                    var share_desc = cutil.share_lack_str(player.curGameRoom) + cutil.get_playing_room_detail(player.curGameRoom);
        ...
        var self = this;
        let extra_panel = this.rootUINode.getChildByName('extra_panel');

        let cancel_btn = extra_panel.getChildByName('extra_cancel_btn');
        let mai_btn = extra_panel.getChildByName('extra_mai_btn');
        let xia_btn = extra_panel.getChildByName('extra_xia_btn');
        cancel_btn.addTouchEventListener(function (sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                player.baseCall("confirmExtraPoints", 0);
                player.curGameRoom.extra_score_state[player.serverSitNum] = const_val.EXTRA_STATE_OP_DISABLE;
                self.updateExtraView(player.serverSitNum, 0, 0, true);
            }
        });
        mai_btn.addTouchEventListener(function (sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                player.baseCall("confirmExtraPoints", const_val.EXTRA_OP_MAI);
                player.curGameRoom.extra_score_state[player.serverSitNum] &= ~const_val.EXTRA_STATE_OP_MAI;
                self.updateExtraView(player.serverSitNum, const_val.EXTRA_OP_MAI, 0, true);
            }
        });

        xia_btn.addTouchEventListener(function (sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                player.baseCall("confirmExtraPoints", const_val.EXTRA_OP_XIA);
                player.curGameRoom.extra_score_state[player.serverSitNum] &= ~const_val.EXTRA_STATE_OP_XIA;
                self.updateExtraView(player.serverSitNum, const_val.EXTRA_OP_XIA, 0, true);
            }
        });
        ...
        // if (player.curGameRoom.curRound <= 0) {
        //     this.update_center_bg_img();
        // }

        this.prepare_flag = 0;  //座位没飞的时候
    },
    ...
    swap_seat:function (swap_list, cbk_fuction) {
        cbk_fuction = cbk_fuction || null;
        swap_list = swap_list || [0,1,2,3];
        ...
        this.playerInfoList = player.curGameRoom.playerInfoList;
        this.curRound = player.curGameRoom.curRound;
        function fly() {
            ...
            for(var i=0; i<swap_list.length; i++){
                ...
                clone_panel.setName("clone_player_info_panel" + player.server2CurSitNum(i));
            ...
            var sum  = 0;
            var game_next_panel = self.rootUINode.getChildByName("gameprepare" + (parseInt(cc.sys.localStorage.getItem("GAME_ROOM_UI")) + 2).toString() + "d_panel");
    ...
    update_center_bg_img:function () {
        var gameMode = cc.sys.localStorage.getItem("GAME_ROOM_UI");
        var p0 = this.rootUINode.getChildByName("center_bg_img0");
        var p1 = this.rootUINode.getChildByName("center_bg_img1");
        p0.setVisible(gameMode == const_val.GAME_ROOM_2D_UI);
        p1.setVisible(gameMode != const_val.GAME_ROOM_2D_UI);
    },
    ...
    update_player_info_panel:function(serverSitNum, playerInfo){
        ...
        if(playerInfo){
            ...
            frame_img.addTouchEventListener(function(sender, eventType){
                if(eventType === ccui.Widget.TOUCH_ENDED){
                    h1global.curUIMgr.gameplayerinfo_ui.show_by_info(playerInfo, serverSitNum + 10, self.prepare_flag);
                }
            });
            cutil.loadPortraitTexture(playerInfo["head_icon"], playerInfo["sex"], function(img){
            ...
            if(player.curGameRoom.curRound > 0){
                ...
                var lose_panel = player_info_panel.getChildByName('lose_panel')
                if (lose_panel) {
                    lose_panel.getChildByName("lose_label").setString(playerInfo['total_job_score'] == undefined ? 0 : playerInfo['total_job_score']);
                }
            }
            ...
            if (player.curGameRoom.curRound > 0) {
                player_info_panel.getChildByName("mai_img").setVisible(false);
                player_info_panel.getChildByName("dealer_img").setVisible(false);
                player_info_panel.getChildByName("king_panel").setVisible(false);
            }
    ...
    layoutExtraPanel: function () {
        ...
        let state_op = curGameRoom.extra_score_state[player.serverSitNum];
        let btns = [cancel_btn];
        if ((state_op & const_val.EXTRA_STATE_OP_XIA) === const_val.EXTRA_STATE_OP_XIA) {
            show_num++;
            btns.push(xia_btn);
        }
        if ((state_op & const_val.EXTRA_STATE_OP_MAI) === const_val.EXTRA_STATE_OP_MAI) {
            show_num++;
            btns.push(mai_btn);
        }

        let width = extra_panel.getContentSize().width;
        if (show_num === 2) {
            for (var i = 0; i < btns.length; i++) {
                btns[i].setPositionX(width / 3 * (i + 1))
            }
        } else if (show_num === 3) {
            for (var i = 0; i < btns.length; i++) {
                btns[i].setPositionX(width / 4 * (i + 1))
            }
        }
    },
     ...
     * 更新买子下子按钮显示状态
     */
    updateExtraState: function () {
        if(!this.is_show){return;}
        let state = true;
        let state_op = const_val.EXTRA_STATE_OP_MAI;
        let player = h1global.entityManager.player();
        if (player && player.curGameRoom) {
            let curGameRoom = player.curGameRoom;
            state_op = curGameRoom.extra_score_state[player.serverSitNum];
            for (var i = 0; i < curGameRoom.playerStateList.length; i++) {
                if (curGameRoom.playerStateList[i] != 1) {
                    state = false;
                    break
                }
            }
        ...
        let mai_btn = extra_panel.getChildByName('extra_mai_btn');
        let xia_btn = extra_panel.getChildByName('extra_xia_btn');
        cancel_btn.setVisible(state);
        if (state && state_op != const_val.EXTRA_STATE_OP_DISABLE) {
            extra_panel.setVisible(true);
            xia_btn.setVisible((state_op & const_val.EXTRA_STATE_OP_XIA) === const_val.EXTRA_STATE_OP_XIA);
            mai_btn.setVisible((state_op & const_val.EXTRA_STATE_OP_MAI) === const_val.EXTRA_STATE_OP_MAI);
            this.layoutExtraPanel();
        } else {
     ...
     * 更新按钮状态和分数
     * @param serverSitNum
     * @param extra_op EXTRA_OP_CANCEL EXTRA_OP_MAI EXTRA_OP_XIA
     * @param score
     * @param onlyUpdateBtn
     */
    updateExtraView: function (serverSitNum, extra_op, score ,onlyUpdateBtn) {
        if(!this.is_show){return;}
        let player = h1global.entityManager.player();
        if (!player || !player.curGameRoom) {
            return;
        ...
        if (serverSitNum == player.serverSitNum) {
            ...
            let mai_btn = extra_panel.getChildByName('extra_mai_btn');
            let xia_btn = extra_panel.getChildByName('extra_xia_btn');
            let state_op = player.curGameRoom.extra_score_state[serverSitNum];
            if (state_op == const_val.EXTRA_STATE_OP_DISABLE || state_op == 0) {
                cancel_btn.color = cc.color(191,191,191);
                cancel_btn.setTouchEnabled(false);
                extra_panel.setVisible(false);
            } else {
                if (extra_op == const_val.EXTRA_OP_XIA) {
                    xia_btn.color = cc.color(191, 191, 191);
                    xia_btn.setTouchEnabled(false);
                } else if (extra_op == const_val.EXTRA_OP_MAI) {
                    mai_btn.color = cc.color(191, 191, 191);
                    mai_btn.setTouchEnabled(false);
                } else if (extra_op == const_val.EXTRA_OP_CANCEL) {
                    cancel_btn.setTouchEnabled(false);
                    mai_btn.setTouchEnabled(false);
                    xia_btn.setTouchEnabled(false);
                    cancel_btn.color = cc.color(191, 191, 191);
        ...
        if (!onlyUpdateBtn) {
            this._selfUpdateExtraScorePanel(player.server2CurSitNum(serverSitNum), extra_op, score);
        }
    },

    updateExtraScorePanel: function (extra_op, scores) {
        if (!this.is_show) {return;}
        let player = h1global.entityManager.player();
        if (!player || !player.curGameRoom) {return;}
        if (player.curGameRoom.curRound > 0) {
            for (var i = 0; i < scores.length; i++) {
                this._selfUpdateExtraScorePanel(player.server2CurSitNum(i), extra_op, scores[i])
            }
        }
    },

    _selfUpdateExtraScorePanel: function (index, extra_op, score) {
        var player = h1global.entityManager.player();
        if (extra_op == const_val.EXTRA_OP_XIA) {
            let extra_score_panel = this.gameprepare_panel.getChildByName('extra_score_panel');
            let score_panel = extra_score_panel.getChildByName("score_panel" + index);
            score_panel.setVisible(score > 0);
            score_panel.getChildByName('num_label').setString('X' + score);
        } else if (extra_op == const_val.EXTRA_OP_MAI) {
            var cur_player_info_panel = this.gameprepare_panel.getChildByName("player_info_panel" + index);
            var mai_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "mai_img");
            if (player.curGameRoom.curRound > 0) {
                mai_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "mai_img");
            } else {
                mai_img = this.rootUINode.getChildByName("clone_player_info_panel" + index).getChildByName("mai_img");
            }
            if(mai_img){
                mai_img.setVisible(true);
            }
        }
    },

    update_extra_left_time:function (extraLeftTime) {
        if (!this.is_show) {return;}
        var player = h1global.entityManager.player();
        if(!player || !player.curGameRoom){
            return;
        }
        extraLeftTime = Math.ceil(extraLeftTime);
        var time_label = this.gameprepare_panel.getChildByName("center_bg_img").getChildByName("time_label");
        if (player.curGameRoom.curRound > 0) {
            time_label = this.gameprepare_panel.getChildByName("center_bg_img").getChildByName("time_label");
        } else {
            // this.update_center_bg_img();
			var gameMode = cc.sys.localStorage.getItem("GAME_ROOM_UI");
			time_label = this.rootUINode.getChildByName("center_bg_img" + gameMode).getChildByName("time_label");
        }
        time_label.setString(extraLeftTime);
        time_label.setVisible(false);
        if (player.curGameRoom.playerStateList.indexOf(0) < 0) {
