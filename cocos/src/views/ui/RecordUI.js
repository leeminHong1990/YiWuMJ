...
var RecordUI = UIBase.extend({
    ...
    update_record_details_small: function (view, itemData, index) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            let score = itemData["round_record"][i]["score"] + itemData["round_record"][i]["job_score"];
    ...
    update_record_details_big: function (view, itemData) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            for (var j = 0; j < round_result.length; j++) {
                total_score += round_result[j]["round_record"][i]["score"] + round_result[j]["round_record"][i]["job_score"];
            }
