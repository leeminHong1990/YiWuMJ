...
var GameRoom2DUI = GameRoomUI.extend({
    ...
    update_player_hand_king: function (index, tile_img) {
        ...
        if(index ===0){
            ...
            tile_img.setPosition(cc.p(-19, -9));
            tile_img.setScaleY(1.04);
        }else if(index ===1){
            tile_img.loadTexture("res/ui/GameRoom2DUI/left_king.png");
        }else if(index ===2){
            tile_img.loadTexture("Mahjong2D/mahjong_tile_player_hand.png", ccui.Widget.PLIST_TEXTURE);
            tile_img.ignoreContentAdaptWithSize(true);
            tile_img.setPosition(cc.p(1106, 117));
            tile_img.color = const_val.mark_king_color;
            mahjong_img.setScaleY(-1);
            mahjong_img.setPosition(cc.p(tile_img.getContentSize().width * 0.5, tile_img.getContentSize().height * 0.42));
