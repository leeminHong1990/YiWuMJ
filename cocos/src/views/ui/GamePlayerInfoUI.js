...
var GamePlayerInfoUI = UIBase.extend({
	ctor:function() {
		...
		this.setLocalZOrder(const_val.GamePlayerInfoZOrder)
	},
	...
	show_by_info:function(info_dict, serverSitNum, prepare_flag){
        prepare_flag = prepare_flag || 0;
	    var idx = serverSitNum >= 10 ? serverSitNum : h1global.entityManager.player().server2CurSitNum(serverSitNum);
		var self = this;
        this.show(function(){
            ...
            if(self.gameplayerinfo_panel.getName() == "prepareplayerinfo_panel"){
            	for(var i = 0 ; i < 4 ; i++ ){
            		if(i == idx - 10){
            		    var curSitNum = h1global.entityManager.player().server2CurSitNum(i);
