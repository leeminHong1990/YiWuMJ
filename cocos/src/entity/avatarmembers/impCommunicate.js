...
var impCommunicate = impBase.extend({
    ...
    recvExpression : function(fromIdx, toIdx, eid){
		...
		fromIdx = h1global.entityManager.player().server2CurSitNum(fromIdx);
        toIdx = h1global.entityManager.player().server2CurSitNum(toIdx);
        if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
	...
	recvVoice : function(serverSitNum, url, record_time){
		...
		wx.downloadVoice({
	      	...
	      	success: function (res) {
				...
				} else if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()){
