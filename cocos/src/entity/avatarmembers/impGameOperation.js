...
var impGameOperation = impCommunicate.extend({
    ...
    startGame: function (dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, extra_mai_list, extra_xia_list) {
		cc.log("startGame");
        cc.log(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, extra_mai_list, extra_xia_list);
		var self = this;
		if(!this.curGameRoom){
			return;
		}
        ...
        this.curGameRoom.extra_mai_list = extra_mai_list;
        this.curGameRoom.extra_xia_list = extra_xia_list;
        ...
        if (this.serverSitNum == dealerIdx) {
            ...
            cutil.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
		...
		this.startActions["GameRoomUI"] = function() {
        	h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "startBeginAnim",self.startTilesList, diceList, self.server2CurSitNum(dealerIdx));
            if (onhookMgr && self.curGameRoom.discard_seconds > 0) {
                onhookMgr.setWaitLeftTime(self.curGameRoom.discard_seconds + const_val.BEGIN_ANIMATION_TIME)
            }
    ...
    seatIndex: function (swap_list) {
        cc.log("seatIndex", swap_list);
        if (!this.curGameRoom) {
            return;
        }
        var self = this;
        ...
        if (const_val.SHOW_SWAP_SEAT) {
            if (h1global.curUIMgr && h1global.curUIMgr.gameroomprepare_ui) {
                if (h1global.curUIMgr.gameroomprepare_ui.is_show && self.curGameRoom.curRound ===0) {
                    h1global.curUIMgr.gameroomprepare_ui.swap_seat(swap_list, function () {
                        h1global.curUIMgr.gameroomprepare_ui.updateExtraState();
                        h1global.curUIMgr.gameroomprepare_ui.updateExtraScorePanel(const_val.EXTRA_OP_XIA, self.curGameRoom.extra_xia_list);
                    });
                } else {
                    h1global.curUIMgr.gameroomprepare_ui.show_prepare(0, enterPlayerInfoList, function () {
                    	if(self.curGameRoom.curRound ===0){
							h1global.curUIMgr.gameroomprepare_ui.swap_seat(swap_list, function () {
								h1global.curUIMgr.gameroomprepare_ui.updateExtraState();
								h1global.curUIMgr.gameroomprepare_ui.updateExtraScorePanel(const_val.EXTRA_OP_XIA, self.curGameRoom.extra_xia_list);
							});
    ...
    readyForNextRound: function (serverSitNum, extraState_list, extra_mai_list, extra_xia_list) {
        if (!this.curGameRoom) {
            return;
        }
        this.curGameRoom.updatePlayerState(serverSitNum, 1);
        this.curGameRoom.extra_score_state = extraState_list;
        this.curGameRoom.extra_mai_list = extra_mai_list;
        this.curGameRoom.extra_xia_list = extra_xia_list;
        if (h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show) {
            h1global.curUIMgr.gameroomprepare_ui.update_player_state(serverSitNum, 1);
            if (this.curGameRoom.curRound > 0) {
                h1global.curUIMgr.gameroomprepare_ui.updateExtraState();
                h1global.curUIMgr.gameroomprepare_ui.updateExtraScorePanel(const_val.EXTRA_OP_XIA, extra_xia_list);
            } else if (this.curGameRoom.playerStateList.indexOf(0) < 0) {
        ...
        if (onhookMgr && this.curGameRoom.extra_wait_time > 0) {
            cc.log("readyForNextRound setWaitExtraTimeLeft", this.curGameRoom.extra_wait_time);
            onhookMgr.setWaitExtraTimeLeft(this.curGameRoom.extra_wait_time)
        }
    },

    notifyPlayerExtraScoreStatus: function (serverSitNum, extra_op, score) {
        if (h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show) {
            h1global.curUIMgr.gameroomprepare_ui.updateExtraView(serverSitNum, extra_op, score);
        }
    },
    ...
    postExtOperation: function (serverSitNum, aid, tile_list, ext_list) {
        if (aid == const_val.OP_DRAW_WIN) {
            if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
				if(ext_list[0] ==1){
                    ...
                    if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                        h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "playOperationEffect", const_val.OP_NORMAL_WIN, serverSitNum);
                        if (this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1) {
                            cc.audioEngine.playEffect("res/sound/voice/male/sound_man_win.mp3");
                        } else {
                            cc.audioEngine.playEffect("res/sound/voice/female/sound_woman_win.mp3");
	...
	postOperation : function(serverSitNum, aid, tileList){
        ...
        var is_king = 0;
        if (this.curGameRoom.discard_king_total[serverSitNum] > 0) {
            is_king = 1;
        }
        this.curGameRoom.updateRecentOperation(serverSitNum, aid, tileList, is_king);
		...
		if(aid == const_val.OP_PASS){
			...
			if (onhookMgr && this.curGameRoom.discard_seconds > 0) {
				var discard_seconds = this.curGameRoom.discard_seconds;
				// if (this.curGameRoom.lastDiscardTile < 0) { //暂时先注掉
				// 	discard_seconds += const_val.BEGIN_ANIMATION_TIME
				// }
				cc.log("OP_DRAW === >", this.curGameRoom.lastDrawTile, this.curGameRoom.discard_seconds, discard_seconds);
				onhookMgr.setWaitLeftTime(discard_seconds);
			}
            if (onhookMgr && const_val.FAKE_COUNTDOWN > 0) {
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
					...
					h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "show_extra_panel", false);
            ...
            if (this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0) {
                this.curGameRoom.discard_king_total[serverSitNum] += 1;
                this.curGameRoom.updateLeftTileNum();
                if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "playOperationEffect", const_val.OP_PIAO, serverSitNum, tileList[0]);
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_player_discard_king", serverSitNum);
                }
            }
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
					...
					h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "show_extra_panel", canWinTiles.length > 0);
			...
			this.curGameRoom.curPlayerSitNum = serverSitNum;
			if(this.serverSitNum == serverSitNum){
                var limit = this.getChowLimitTiles(tileList);
                for (var i = 0; i < limit.length; i++) {
                    this.add_limit_tiles(serverSitNum, const_val.OP_DISCARD, limit[i]);
                }
				...
				this.curGameRoom.upTilesOpsList[this.serverSitNum].push([{"opId":aid, "tiles":tileList.concat(), "fromIdx":this.curGameRoom.lastDiscardTileFrom,"king":is_king}]);
				if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()){
				...
				this.curGameRoom.upTilesOpsList[serverSitNum].push([{"opId":aid, "tiles":tileList.concat(), "fromIdx":this.curGameRoom.lastDiscardTileFrom,"king":is_king}]);
			}
			...
			this.curGameRoom.curPlayerSitNum = serverSitNum;
			if (onhookMgr && this.curGameRoom.discard_seconds > 0) {
				cc.log('OP_PONG=== > ', this.curGameRoom.discard_seconds)
			...
			if(this.serverSitNum == serverSitNum){
				...
				this.curGameRoom.upTilesOpsList[this.serverSitNum].push([{"opId":aid, "tiles":[tileList[0]], "fromIdx":this.curGameRoom.lastDiscardTileFrom,"king":is_king}]);
				if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "show_extra_panel", false);
					h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "unlock_player_hand_tiles");
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_operation_panel", this.getPongKongOpDict(), const_val.SHOW_DO_OP);
				...
				this.curGameRoom.upTilesOpsList[serverSitNum].push([{"opId":aid, "tiles":[tileList[0]], "fromIdx":this.curGameRoom.lastDiscardTileFrom,"king":is_king}]);
			}
			var lastDiscardTileFrom = this.curGameRoom.lastDiscardTileFrom;
			if(lastDiscardTileFrom >= 0){
				this.curGameRoom.lastDiscardTile = 0;
			...
			if(this.serverSitNum == serverSitNum){
				...
				this.curGameRoom.upTilesOpsList[this.serverSitNum][kongIdx].push({"opId":aid, "tiles":[tileList[0]], "fromIdx":this.serverSitNum,"king":is_king});
			} else {
				// 手牌全是0，任意删除1张即可
                this.removeHandTile(serverSitNum, tileList[0], 1);
				this.curGameRoom.upTilesList[serverSitNum][kongIdx].push(tileList[0]);
				this.curGameRoom.upTilesOpsList[serverSitNum][kongIdx].push({"opId":aid, "tiles":[tileList[0]], "fromIdx":serverSitNum,"king":is_king});
			}
			var lastDiscardTileFrom = this.curGameRoom.lastDiscardTileFrom;
			var lastDiscardTile = this.curGameRoom.lastDiscardTile;
			// if(lastDiscardTileFrom >= 0 && this.serverSitNum != serverSitNum){
			...
			if(this.serverSitNum == serverSitNum){
				...
				this.curGameRoom.upTilesOpsList[this.serverSitNum].push([{"opId":aid, "tiles":[tileList[3]], "fromIdx":this.serverSitNum,"king":is_king}]);
			} else {
				// 手牌全是0，任意删除4张即可
                this.removeHandTile(serverSitNum, tileList[3], 4);
				this.curGameRoom.upTilesList[serverSitNum].push(tileList);
				this.curGameRoom.upTilesOpsList[serverSitNum].push([{"opId":aid, "tiles":[tileList[3]], "fromIdx":serverSitNum,"king":is_king}]);
			}
			if (h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                // if (this.serverSitNum == serverSitNum){
                //     h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_operation_panel", this.getPongKongOpDict(), const_val.SHOW_DO_OP);
		...
		if(this.serverSitNum != serverSitNum && h1global.curUIMgr && h1global.curUIMgr.roomLayoutMgr){
	...
	doOperation : function(aid, tileList){
        ...
        // discard_seconds > 0自己的操作直接本地执行，不需要广播给自己
        if (this.curGameRoom.discard_seconds <= 0) {
	...
	roundResult : function(roundRoomInfo){
		...
		for(var i = 0; i < playerInfoList.length; i++){
			...
			this.curGameRoom.playerInfoList[i]["total_job_score"] = playerInfoList[i]["total_job_score"];
		}
		var anim_end_num = 0;
        var self = this;
        ...
        if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
            h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "play_result_anim", playerInfoList, function () {
                anim_end_num += 1;
                callbackfunc()
            });
        }
	...
	finalResult : function(finalPlayerInfoList, roundRoomInfo){
        ...
        if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
            h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "play_result_anim", roundRoomInfo["player_info_list"], function () {
                anim_end_num += 1;
                callbackfunc()
            });
        }
    ...
    subtotalResult:function (finalPlayerInfoList) {
        ...
        var curGameRoom = player.curGameRoom;
        if (h1global.curUIMgr.result_ui) {
            h1global.curUIMgr.result_ui.show_by_info(finalPlayerInfoList, curGameRoom, player.serverSitNum);
        }
    },
