...
var impRoomOperation = impGameRules.extend({
    ...
    createRoom: function (param_dict) {
        cc.log("createRoom:", param_dict);
        this.baseCall("createRoom", param_dict);
    },
	...
	enterRoomFailed : function(err){
		...
		if(err === const_val.ENTER_FAILED_ROOM_NO_EXIST){
	...
	handleReconnect : function(recRoomInfo){
		...
		if (onhookMgr && this.curGameRoom.discard_seconds > 0) {
			cc.log("handleReconnect setWaitLeftTime", recRoomInfo["waitTimeLeft"])
			onhookMgr.setWaitLeftTime(recRoomInfo["waitTimeLeft"])
		}

		if (onhookMgr && this.curGameRoom.waitExtraTimeLeft > 0) {
			cc.log("handleReconnect setWaitExtraTimeLeft", recRoomInfo["waitExtraTimeLeft"])
	...
	voteDismissResult : function(serverSitNum, vote){
		...
		if(this.curGameRoom){
			...
			// }
            if (vote_disagree_num >= 1 || vote_disagree_num >= this.curGameRoom.playerInfoList.length / 2) {
                ...
                return;
            } else if (vote_agree_num == this.curGameRoom.playerInfoList.length) {
                // this.quitRoom();
            }
			if(h1global.curUIMgr.applyclose_ui && h1global.curUIMgr.applyclose_ui.is_show){
