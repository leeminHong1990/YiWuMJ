...
const_val.GameRoomInfoZOrder = 99;
const_val.GameRoomPrepareZOrder = 90;
const_val.GamePlayerInfoZOrder = 95;
const_val.AudioRecordZOrder = 95;
...
// Note: 为了加入飘的效果加入的一个操作id,只在客户端识别
const_val.OP_PIAO		  = 210;
const_val.OP_NORMAL_WIN		  = 211;
...
const_val.WINDS_DRAGONS = [71, 72, 73, 74, 75, 76, 77];
...
const_val.DISMISS_ROOM_WAIT_TIME = 200; // 申请解散房间后等待的时间, 单位为秒
...
const_val.mark_disable_color = cc.color(191,191,191);
...
const_val.EXPRESSION_ANIM_LIST = ["egg", "kiss", "cheers", "money"];
const_val.EXPRESSION_ANIMNUM_LIST = [15, 23, 15, 10];
...
// 规则
const_val.NORMAL_ROUND_MODE = 0;	// 打捆
const_val.DOUBLE_ROUND_MODE = 1;	// 长跑
const_val.ROUND_MODE = [const_val.NORMAL_ROUND_MODE, const_val.DOUBLE_ROUND_MODE];
// 局数
const_val.ROUND = [8, 16, 24];
// 封顶
const_val.MAX_LOSE = [9999, 100];

//以分数为限制的分数
const_val.DEFAULT_GAME_SCORE = 100;  // 以分数为限制的默认分数
const_val.GAME_SCORE = [const_val.DEFAULT_GAME_SCORE];

// 基础分
const_val.BASE_SCORE_MODE_ZERO = 0;	//庄家9分，闲家7分
const_val.BASE_SCORE_MODE_ONE = 1;		//庄家13分，闲家11分
const_val.BASE_SCORE_MODE = [const_val.BASE_SCORE_MODE_ZERO, const_val.BASE_SCORE_MODE_ONE];
// 承包
const_val.MODE_NOT_JOB = 0;	//不承包
const_val.MODE_JOB = 1;		//承包以及反承包
const_val.JOB_MODE = [const_val.MODE_NOT_JOB, const_val.MODE_JOB];
...
// 财神
const_val.KING_MODE_FIXED = 0;  // 固定财神
const_val.KING_MODE_RANDOM = 1;  // 随机财神
const_val.KING_FIXED_TILE = const_val.DRAGON_WHITE;  // 固定白板财神
/* ########################################################################################### */
...
const_val.CREATE_FAILED_PERMISSION_DENIED = -4;	// 不是代理, 不能代开房
const_val.CREATE_FAILED_NET_SERVER_ERROR = -8;	// 访问外部网络结果失败
...
// 开局后有m张牌不能被摸 （不飘 单飘 双飘 三飘）
const_val.IDLE_END_TILES = [17 * 2, 7 * 2, 0, 0];

const_val.EXTRA_OP_CANCEL = 0; // 取消操作
const_val.EXTRA_OP_MAI = 1;  // 买子操作
const_val.EXTRA_OP_XIA = 2;  // 下子操作

const_val.EXTRA_STATE_OP_NONE = 0;
const_val.EXTRA_STATE_OP_DISABLE = 1 << 0;  // 禁止买子和下子
const_val.EXTRA_STATE_OP_MAI = 1 << 1;  // 可以买子
const_val.EXTRA_STATE_OP_XIA = 1 << 2;  // 可以下子
...
// 今日日活
const_val.DAU_TODAY = 1;
// 昨日日活
const_val.DAU_YESTERDAY = 2;
