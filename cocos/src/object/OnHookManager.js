...
OnHookManager.prototype.update = function(interval){
    ...
    if (this.waitExtraTimeLeft != null) {
        this.waitExtraTimeLeft = this.waitExtraTimeLeft - interval;
        if (h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show) {
            if (this.waitExtraTimeLeft <= 0) {
                h1global.curUIMgr.gameroomprepare_ui.update_extra_left_time(0);
                this.waitExtraTimeLeft = null;
            } else {
                h1global.curUIMgr.gameroomprepare_ui.update_extra_left_time(this.waitExtraTimeLeft);
            }
        }
    }
};
...
OnHookManager.prototype.setWaitExtraTimeLeft = function(waitExtraTimeLeft){
    // 单位是秒
    this.waitExtraTimeLeft = waitExtraTimeLeft;
};