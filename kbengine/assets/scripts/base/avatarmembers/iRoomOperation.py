...
class iRoomOperation(object):
	...
	def createRoom(self, create_dict):
		...
		if self.req_entering_room:
			DEBUG_MSG("iRoomOperation: createRoom failed; entering or creating room")
			return
		if self.isInRoom:
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, {"round_mode": round_mode, "pay_mode": pay_mode})
	...
	def startGame(self, dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, extra_mai_list, extra_xia_list):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, extra_mai_list, extra_xia_list)
	...
	def postExtOperation(self, idx, aid, tile_list, ext_list):
		if getattr(self, 'client', None):
			self.client.postExtOperation(idx, aid, tile_list, ext_list)

	# c2s
	def confirmExtraPoints(self, extra):
		# NOTE: 当需要用到多个值的时候需要验证参数 现在不需要
		self.room.confirm_extra_points(self, extra)

	def notifyPlayerExtraScoreStatus(self, serverSitNum, extra_op, score):
		if getattr(self, 'client', None):
			self.client.notifyPlayerExtraScoreStatus(serverSitNum, extra_op, score)
	...
	def readyForNextRound(self, idx, state_list, extra_mai_list, extra_xia_list):
		"""
		:param idx:
		:param state_list: 是否可以买子或下子
		:param extra_mai_list
		:param extra_xia_list
		:return:
		"""
		if getattr(self, 'client', None):
			self.client.readyForNextRound(idx, state_list, extra_mai_list, extra_xia_list)
	...
	# s2c
	def seatIndex(self, swap_list):
		if self.room:
			getattr(self, 'client', None) and self.client.seatIndex(swap_list)
