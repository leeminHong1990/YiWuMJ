...
DISMISS_ROOM_WAIT_TIME 								= 200	# 申请解散房间后等待的时间, 单位为秒
...
CREATE_FAILED_PERMISSION_DENIED = -4 # 不是代理, 不能代开房
CREATE_FAILED_NET_SERVER_ERROR = -8  # 访问外部网络结果失败
...
# 局数
ROUND = (8, 16, 24)
ROUND_MODE_SCORE = 0  # 分数为0 结束牌局
ROUND_MODE_ROUND = 1  # 按局数来判断结束
ROUND_MODE = (ROUND_MODE_SCORE, ROUND_MODE_ROUND)

# 以分数为限制的分数
DEFAULT_GAME_SCORE = 100  # 以分数为限制的默认分数
GAME_SCORE = (DEFAULT_GAME_SCORE,)

# 基础分
BASE_SCORE_MODE_ZERO = 0	#庄家9分，闲家7分
BASE_SCORE_MODE_ONE = 1		#庄家13分，闲家11分
BASE_SCORE_MODE = (BASE_SCORE_MODE_ZERO, BASE_SCORE_MODE_ONE)
# 承包
MODE_NOT_JOB = 0	#不承包
MODE_JOB = 1		#承包以及反承包
JOB_MODE = (MODE_NOT_JOB, MODE_JOB)
# 封顶
MAX_LOSE = (9999, 100)
# 出牌时限
...
KING_MODE_FIXED = 0  # 固定财神
KING_MODE_RANDOM = 1  # 随机财神
KING_FIXED_TILE = DRAGON_GREEN  # 固定白板财神
KING_MODES = [KING_MODE_FIXED, KING_MODE_RANDOM]

# 开局后有m张牌不能被摸 （不飘 单飘 双飘 三飘）
IDLE_END_TILES = (17 * 2, 7 * 2, 0, 0)

# 基础分数
BASE_SCORE_MODE_ZERO = 0  # 庄家9分，闲家7分
BASE_SCORE_MODE_ONE = 1  # 庄家13分，闲家11分
# (庄，闲)
BASE_SCORE_LIST = [(9, 7), (13, 11)]

MAX_EXTRA_SCORE = 64  # 最大下子分数

EXTRA_WAIT_TIME = 15  # 下子和买子的等待的时间, 单位为秒
EXTRA_OP_CANCEL = 0  # 取消操作
EXTRA_OP_MAI = 1  # 买子操作
EXTRA_OP_XIA = 2  # 下子操作
EXTRA_WAIT_ANIM_TIME = 4  # 显示买子和下子前客户端等待动画时间

EXTRA_STATE_OP_NONE = 0
EXTRA_STATE_OP_DISABLE = 1 << 0  # 禁止买子和下子
EXTRA_STATE_OP_MAI = 1 << 1  # 可以买子
EXTRA_STATE_OP_XIA = 1 << 2  # 可以下子
